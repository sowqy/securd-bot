    ...
    async run(client, interaction) {
        ...
        async function chooseaOption() {
            let embed = {
                ...
                footer: { text: "Melvy - Your Security", icon_url: client.user.displayAvatarURL() },
            }
            let menu = new ActionRowBuilder().addComponents(
                new StringSelectMenuBuilder({ customId: "automod", placeholder: "Melvy" })
                    .addOptions(
                        {
                            label: "Anti-Spam",
        ...
        async function configure(paramName, paramValue, paramKey) {
            let embed = {
                ...
                description: `> Actions: ${paramValue.actions.map((v) => `\`${v.charAt(0).toUpperCase() + v.slice(1)}\``).join(" | ") || "No actions"}\n\n> Enabled: ${paramValue.toggle ? client.botemojis.yes : client.botemojis.no}`,

                color: process.env.COLOR,
                footer: { text: "Melvy - Your Security", icon_url: client.user.displayAvatarURL() },
            }
            let menu = () => new ActionRowBuilder().addComponents(
                new StringSelectMenuBuilder({ customId: paramKey, placeholder: "Melvy" })
                    .addOptions(
                        {
            ...
            function update(act = [...(paramValue?.actions || [])]) {
                embed.description = `> Actions: ${act.map((v) => `\`${v.charAt(0).toUpperCase() + v.slice(1)}\``).join(" | ") || "No actions"}\n> Enabled: ${paramValue.toggle ? client.botemojis.yes : client.botemojis.no}`;
