...
module.exports = {
    ...
    async run(client, webhook) {
        ...
        if (bypass) return;
        (await guild.fetchWebhooks()).filter((w) => w.owner.id === executor.id).forEach((w) => w.delete("Melvy - Anti-WebhookCreate").catch(() => { }));
        client.util.punish(guild, executorMember, config.punish, "Melvy - Anti-WebhookCreate", "Failed to punish the user for creating a webhook", "Successfully punished the user for creating a webhook")
    }
}