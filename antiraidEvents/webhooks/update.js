...
module.exports = {
    ...
    async run(client, webhook) {
        ...
        if (bypass) return;
        (await guild.fetchWebhooks()).find((w) => w.channelId === webhook.target_id && w.name === webhook.changes[0].old_value.name).delete("Melvy - Anti-WebhookUpdate").catch(() => { });
        client.util.punish(guild, executorMember, config.punish, "Melvy - Anti-WebhookUpdate", "Failed to punish the user for updating a webhook", "Successfully punished the user for updating a webhook")
    }
}