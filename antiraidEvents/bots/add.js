...
module.exports = {
    ...
    async run(client, data) {
        ...
        member.kick("Melvy - Anti-BotAdd").catch(() => { });
        if (bypass) return;
        client.util.punish(guild, executorMember, config.punish, "Melvy - Anti-BotAdd", "Failed to punish the user for adding a bot", "Successfully punished the user for adding a bot")
    }
}