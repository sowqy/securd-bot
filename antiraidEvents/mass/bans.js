...
module.exports = {
    ...
    async run(client, ban) {
        ...
        if (data.length + 1 >= number) {
            client.util.punish(guild, executorMember, config.punish, "Melvy - Anti-MassBan", "Failed to punish the user for banning multiple users", "Successfully punished the user for banning multiple users")
            map.delete(`${guild.id}-${executor.id}`)
            data.forEach((id) => {
                guild.members.unban(id, "Melvy - Anti-MassBan").catch(() => { })
            })
        } else {
